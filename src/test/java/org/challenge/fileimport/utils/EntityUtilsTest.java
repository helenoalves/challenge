...
public class EntityUtilsTest {
  ...
  public void shouldCreateLines() {
    ...
    String[] result = StringUtils.split(oneLine, EntityUtils.SEPARATOR);
  ...
  public void shouldCreateEntities() {
    ...
    Assert.assertEquals(Integer.valueOf(10), outputFileContent.getSalesdata().stream().findAny().get().getSaleId());
    Assert.assertEquals(Integer.valueOf(10), outputFileContent.getSalesdata().stream().findAny().get().getSaleId());
    Assert.assertEquals(3, outputFileContent.getSalesdata().stream().findAny().get().getItems().size());
  }

  @Test
  public void shouldCreateSalesman() {
    String salesmanLine = "001ç1234567891234çPedroç50000";
    Salesman salesmanCreated = EntityUtils.getInstance().createSalesman(salesmanLine);
  ...
  public void shouldCreateClient() {
    ...
    Client clientCreated = EntityUtils.getInstance().createClient(clientLine);
  ...
  public void shouldCreateSaleData() {
    ...
    SaleData saleDataCreated = EntityUtils.getInstance().createSaleData(saleDataLine);

    Assert.assertEquals("Pedro", saleDataCreated.getSalesmanName());
    Assert.assertEquals(BigDecimal.valueOf(2.50).setScale(2), saleDataCreated.getItems().get(1).getPrice());
  }
}